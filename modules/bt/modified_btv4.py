...
locomotion_timeout = config.get('agents', {}).get('locomotion_timeout', 10.0)  # seconds
...
complementary nodes in between.
'''
...
class HybridLocomotionNode(SyncAction):
    ...
    def __init__(self, name, agent):
        self.current_waypoint = self.set_common_waypoint()
        self.timeout = 0.0  # Timer to track duration at current waypoint
        self.leader_id = 0  # Assume agent 0 is leader.
    ...
    def _hybrid_locomotion(self, agent, blackboard):
        ...
        if agent.agent_id == self.leader_id:
            ...
            leader = next((a for a in agent.get_all_agents() if a.agent_id == self.leader_id), None)
            if leader and 'common_waypoint' in leader.blackboard:
                self.current_waypoint = leader.blackboard['common_waypoint']
            else:
                self.current_waypoint = None
        
        # Check Timeout
        if self.timeout > locomotion_timeout:
            self.timeout = 0.0
            return Status.FAILURE
        
        # Perform locomotion behavior and keep timer running
        self.timeout += sampling_time
        agent.follow(self.current_waypoint, weight=1.0)
        return Status.RUNNING  # Continue flocking over ticks
